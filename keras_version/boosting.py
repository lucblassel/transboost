...
def batchBooster(full_model,x_train,y_train,x_val,y_val,params_temp,epochs_target,lr_target,threshold,layerLimit,times,bigNet,originalSize,resizeFactor,proba_threshold,step,**kwargs):
	"""
	romain.gautron@agroparistech.fr
	"""
	train_length = len(x_train)
...
#		print( "boosting step number "+str(time))
		...
		if time % step == 0:        
			x_train_target,y_train_target,x_val_target,y_val_target,x_test_target,y_test_target = from_generator_to_array(path_to_train,path_to_validation,trainNum_target,valNum_target,testNum_target,**params_temp)
			predicted_classes = prediction_boosting(x_test_target,model_list,alpha_list,current_model,**params_temp)
...
def main():
	...
	try:
		...
		model_list, _ , alpha_list, model_returned = batchBooster(full_model,x_train_target,y_train_target,x_val_target,y_val_target,params,**params)
